...
public class ColoredBSSW {
    private final String name;
    private final Map<DyeColor, BSSW> map = new HashMap<>();

    public ColoredBSSW(String name, AbstractBlock.Settings baseSettings) {
        this.name = name;

        forEachDyeColor(color -> map.put(color,
                new BSSW(color.getName() + "_" + name,
    ...
    public void register(Registrar r) {
        ...
        ItemGroupHelper.append(ItemGroups.COLORED_BLOCKS, e -> {
            ...
            colors.forEach(bssw -> e.add(bssw.block));
            colors.forEach(bssw -> e.add(bssw.slab));
            colors.forEach(bssw -> e.add(bssw.stairs));
            colors.forEach(bssw -> e.add(bssw.wall));
        });
    }
}
